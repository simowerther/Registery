...
.setDescription(`${message.author} Yetkin mi var amip bi sen akıllısın`)
.setColor('2F3136')
.setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));


let famelcim = message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size == 0)
...
.setColor("2F3136")
.setDescription("Sunucuda rolü olmayan \`"+ famelcim.size +"\` kişiye \`Kayıtsız\` rolü verildi")
...
.setFooter(config.Footer)
.setColor("2F3136")
.setDescription("Sunucumuzda rolü olmayan \`"+ famelcim.size +"\` kişi var.")
