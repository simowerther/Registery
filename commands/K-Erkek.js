...
module.exports.run = async (client, message, args) => {
  ...
  if (!member) return message.channel.send(embed
    .setDescription("Geçerli bir \`@famel/ID\` belirtmelisin"))
  ...
  await member.roles.add(client.config.boyRoles).catch(err => { return undefined; });
  await member.roles.remove(client.config.unregisterRoles).catch(err => { return undefined; });

  let kanal = member.guild.channels.cache.find(r => r.id === config.chatKanal);
  kanal.send(`${member} Aramıza katıldı, Hoşgeldin!`).then(x => x.delete({ timeout: 15000 }));
  let log = member.guild.channels.cache.find(r => r.id === config.reglog);
  log.send(`${member} - (\`${member.id}\`) aramıza \`erkek\` olarak katıldı! Kayıt Eden Yetkili: \`${message.author.tag}\``)
