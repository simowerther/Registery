...
moment.locale("tr");
kanal.send(`<a:starrr:969641777705398415> Sunucumuza birisi katıldı! Sevgili ${member} seninle birlikte **${member.guild.memberCount}** kişi olduk.
  
<a:starrr:969641777705398415> Gördüğüm kadarıyla hesabın **${moment(member.user.createdTimestamp).format("LLL")}** tarihinde oluşturulmuş.

<a:starrr:969641777705398415> Bu da demek oluyor ki hesabın **Güvenli** <a:onay:969641746998894662> <a:sustursana:943811600253583360>  

<a:black:954727523034497064> Sunucu kurallarımız <#969309966924120104> kanalında belirtildi. 

<a:black:954727523034497064> Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.

<a:giris:954637176317050930> Sol tarafta bulunan **V. Confirmed** odalarından birine girerek ve .tag yazarak tagımızı alarak kayıt işlemini gerçekleştirebilirsin.

<a:mo:943810209158135848> Sunucumuza hoş geldinnn! `)})
  
client.on("message", async message => {
  if (message.content === "!famelcim") {if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Bu komutu kullanabilmek için **Yönetici** yetkisine sahip değilsin!");
    client.emit(
      "guildMemberAdd",
...
client.on("guildMemberAdd", async famelcim => {
const kanal = famelcim.guild.channels.cache.find(r => r.id === config.HoşgeldinKanalID);
const tarih = new Date().getTime() - famelcim.user.createdAt.getTime();
   if (tarih < 1000*360*247 ) {
   famelcim.roles.add(config.ŞüpheliHesap)
   famelcim.roles.remove(config.unregisterRoles)
   kanal.send(`${famelcim} ${famelcim.id} Adlı kullanıcının hesabı yeni açıldığı için jaile atıldı.`)
   famelcim.send(`Hesabın 7 günden az bi sürede açıldığı için jaile atıldın. Eğer bir yanlışlık olduğunu düşünüyorsan yanda bulunan yetkililere yazabilirsin.`)}
}); 
...
hours = 12;
  ...
  const webhook = new Discord.WebhookClient('969315198420799528','Mvvq2b0qVFV-uN1inFe0WOnyQym5uF4-X6uU5QNGaEw_3BPx5M5NftlQiAG89CQNSq4c');
      let sonuc = [`:inbox_tray: **${member.user.tag}** sunucuya katıldı! Saat: \`${time}\``];
...
client.on("message", async message => {
  if(message.content === `.yetkilisay`) {
      message.channel.send(message.guild.members.cache.filter(member => member.roles.highest.position >= message.guild.roles.cache.get("969309895964885022").position && !member.voice.channel && member.presence.status !== "offline").map(qwe => `<@!${qwe.id}>,`).join("\n"))
message.guild.members.cache.filter(member => member.roles.highest.position >= message.guild.roles.cache.get("969309895964885022").position && !member.voice.channel && member.presence.status !== "offline").forEach(a => a.send(`Ses kanallarına girmelisin.`))
  }
  if(message.content === `.ysay`) {
      message.channel.send(message.guild.members.cache.filter(member => member.roles.highest.position >= message.guild.roles.cache.get("969309895964885022").position && !member.voice.channel && member.presence.status !== "offline").map(qwe => `<@!${qwe.id}>,`).join("\n"))
message.guild.members.cache.filter(member => member.roles.highest.position >= message.guild.roles.cache.get("969309895964885022").position && !member.voice.channel && member.presence.status !== "offline").forEach(a => a.send(`Ses kanallarına girmelisin.`))
  }
})
...
hours = 12; 
  ...
  const webhook = new Discord.WebhookClient('969315198420799528','Mvvq2b0qVFV-uN1inFe0WOnyQym5uF4-X6uU5QNGaEw_3BPx5M5NftlQiAG89CQNSq4c');
      let sonuc = [`:outbox_tray: **${member.user.tag}** sunucudan çıkış yaptı! Saat: \`${time}\``];

      let result = Math.floor((Math.random() * sonuc.length));
  webhook.send(sonuc[result])
});
client.on("guildMemberAdd", member => {
if(member.user.username.includes("bombosunnssun") && (member.user.username.includes("bombosusuunnn")) && (member.user.username.includes("bombosusunnn") && (member.user.username.includes("bombusuunnnnnsus")))){
...
client.on("guildMemberAdd", member => {
const famelcimcik = member.guild.channels.cache.find(r => r.id === config.HoşgeldinKanalID);
if(member.user.username.includes(config.tag)){
member.roles.add(config.Tagges)
famelcimcik.send(`${member} (\`${member.id}\`) Sunucuya Taglı Bir Şekilde Giriş Yaptı!`)}
})

async function yasaklitag() {
  let guild = client.guilds.cache.get(config.guildID);
  let data = await db.get(`yasaklitaglar.${guild.id}`) || []  
guild.members.cache.filter(s => data.some(x => s.user.username.includes(x)) && !s.roles.cache.has("969309932203692032")).forEach(m => m.roles.set(['969309932203692032']))}
setInterval(async() => {
yasaklitag()
}, 60000)
